...
module.exports = {
	...
	async execute(interaction, noblox) {
        ...
        if (!interactionStringArg) {
            axios.get('https://api.rowifi.link/v1/users/' + interaction.user.id)
        ...
        function getRobloxUsername(value, roblox) {
            if (!value) {
                return interaction.reply(`Sorry ${interaction.user.username}, but you do not seem to be verified in the rover database so that I can fetch your account. Please by saying \`\`/verify\`\``)
            } else {
                var usernameParam = {
                    "userIds": [
                      roblox
            ...
            function createButtonBuilder(interactionembed) {
                ...
                profile_to_groups_collector.on(`collect`, async i => {
                    ...
                    if (i.customId === `profile_to_groups`) {
                        ...
                        i.editReply({ content: ``, embeds: [newinteractionembed], components: [group_profile_actionrowbuilder] })
                            .then(() => console.log(`Reply edited.`))
                            .catch(console.error);
                    }
                    ...
                    if (i.customId === `back_to_profile`) {
                        ...
                        wait(1000)
                        i.editReply({ content: ``, embeds: [interactionembed], components: [normal_profile_actionrowbuilder] })
                            .then(() => console.log(`Reply edited.`))
                            .catch(console.error);
                    }
                })
                profile_to_groups_collector.on(`end`, collected => {
                    ...
                    return interaction.editReply({ content: `Sender pressed these embeds buttons ${collected.size} times!`, components: [] })
                })
